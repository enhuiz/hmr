...
class UPerNetG(nn.Module):
    def __init__(self, num_class=1, ngf=16, pool_scales=(1, 2, 3, 6)):
        super().__init__()
        self.encoder = ResNet(ngf)

        fc_dim = ngf * 8
        fpn_inplanes = (ngf, ngf * 2, ngf * 4, ngf * 8)
        fpn_dim = ngf * 8
...
class UNetG(nn.Module):
    ...
    def __init__(self, input_nc=1, output_nc=1, num_downs=4, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False):
